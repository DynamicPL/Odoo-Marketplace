...
/* License URL : https://store.webkul.com/license.html/ */
    ...
    'margin-right': '10px'});
        ...
        $(document).on("click","#mp_t_and_c", function()
		{
            ...
            jsonrpc("/mp/terms/and/conditions",{
                'mp_t_and_c': mp_t_and_c,
            })
            .then(function (modal) {

                var $modal = $(modal);
                $('#mp_t_and_c_modal  .mp_wrap_content').html(mp_t_and_c)
            ...
            if(profile_url != '' && /^[0-9]+$/.test(profile_url)) {
              ...
              $('#profile_url_error').show();
                ...
                if(profile_url != '' && profile_url.match(/^[-_][a-zA-Z0-9-_]*$/) || profile_url.match(/^[a-zA-Z0-9-_]*[-_]$/)){
                    ...
                    jsonrpc("/profile/url/handler/vaidation",{'url_handler': profile_url})
    				.then(function (vals)
    				{
	    ...
	    $('#recently_product_tab').click(function(e){
	    	...
	    	if (temp === 1){
				...
				jsonrpc("/seller/shop/recently-product/", {'shop_id': shop_id})
				.then(function (vals)
				{
		            var $modal = $(vals);
		            temp = temp+1;
	    ...
	    $('#profile_recently_product_tab').click(function(e){
			...
			if (temp === 1){
				$('.mp-box-review_loader').show();
                jsonrpc("/seller/profile/recently-product/", {'seller_id': seller_id})
    			.then(function (vals)
    			{
    	            var $modal = $(vals);
                    temp = temp+1;
       	...
       	$('.sol-info').on('click', function()
        {
        	...
        	// var ajax = require('web.ajax');
        	var self = this;
			jsonrpc("/track/sol", {'sol_id': order_line})
			.then(function (vals)
			{
				if (vals)
