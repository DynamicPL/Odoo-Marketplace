...
class AccountMove(models.Model):
    ...
    seller_invoice_number = fields.Char(
        string='Seller Invoice Number', readonly=True, translate=True)
        ...
        for invoice_obj in self:
            ...
            if invoice_obj.move_type in ['out_invoice', 'out_refund']:
                for invoice_line_obj in invoice_obj.invoice_line_ids:
                    ...
                    if seller_id:
                        commission_type = self.env['res.config.settings'].get_mp_global_field_value('commission_calculation_type')
                        price = invoice_line_obj.price_total
                        if commission_type == 'admin_bearer':
                            price = invoice_line_obj.price_unit
                        seller_amount = self.calculate_commission(price, seller_id)
                        invoice_line_obj.seller_commission = price - seller_amount
        ...
        if sellers_dict:
            ...
            for seller in sellers_dict["seller_ids"].keys():
                ...
                for amount in sellers_dict["seller_ids"][seller]["invoice_line_payment"]:
                    total_amount += amount
                mp_currency_obj = self.env["res.currency"].browse(self.env['ir.default'].sudo()._get('res.config.settings', 'mp_currency_id')) or self.env.user.currency_id
                vals.update({
                    "invoiced_amount": total_amount,
                    "payable_amount": invoice_currency._convert(total_amount, mp_currency_obj),
