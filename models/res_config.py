...
class ResConfigSettings(models.TransientModel):
    ...
    def get_journal_id(self):
        ...
        return obj[0] if obj else self.env["account.journal"].sudo()
    ...
    mp_payment_waiting_period = fields.Integer(string="Payout Waiting Period (Days)", default=7, help="Number of days to wait before a seller's earnings can be paid out.")
    ...
    commission_calculation_type = fields.Selection([('admin_bearer','Admin Bearer'),
                                                   ('split','Split between admin and seller')], help="If the Admin bearer option is selected, the commission will be calculated without tax; if the option split between admin and seller is selected, then the commission will be calculated with tax.", required=True)
    ...
    def set_values(self):
        ...
        self.env['ir.default'].sudo().set('res.config.settings', 'mp_payment_waiting_period', self.mp_payment_waiting_period)
        ...
        self.env['ir.default'].sudo().set('res.config.settings', 'commission_calculation_type', self.commission_calculation_type)

    def get_values(self):
        res = super(ResConfigSettings, self).get_values()

        mp_auto_product_approve = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_auto_product_approve')
        internal_categ = self.env['ir.default'].sudo()._get('res.config.settings', 'internal_categ') or self._default_category().id
        mp_warehouse_id = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_warehouse_id', company_id=True)
        mp_location_id = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_location_id', company_id=True) or self._default_location().id
        mp_auto_approve_qty = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_auto_approve_qty')
        auto_approve_seller = self.env['ir.default'].sudo()._get('res.config.settings', 'auto_approve_seller')
        mp_commission = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_commission')
        mp_seller_payment_limit = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_seller_payment_limit')
        mp_next_payment_request = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_next_payment_request')
        mp_payment_waiting_period = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_payment_waiting_period')
        enable_notify_admin_4_new_seller = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_admin_4_new_seller')
        enable_notify_seller_4_new_seller = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_seller_4_new_seller')
        enable_notify_admin_on_seller_approve_reject = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_admin_on_seller_approve_reject')
        enable_notify_seller_on_approve_reject = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_seller_on_approve_reject')
        enable_notify_admin_on_product_approve_reject = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_admin_on_product_approve_reject')
        enable_notify_seller_on_product_approve_reject = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_seller_on_product_approve_reject')
        enable_notify_seller_on_new_order = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_seller_on_new_order')
        notify_admin_4_new_seller_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_admin_4_new_seller_m_tmpl_id')
        notify_seller_4_new_seller_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_seller_4_new_seller_m_tmpl_id')
        notify_admin_on_seller_approve_reject_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_admin_on_seller_approve_reject_m_tmpl_id')
        notify_seller_on_approve_reject_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_seller_on_approve_reject_m_tmpl_id')
        notify_admin_on_product_approve_reject_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_admin_on_product_approve_reject_m_tmpl_id')
        notify_seller_on_product_approve_reject_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_seller_on_product_approve_reject_m_tmpl_id')
        notify_seller_on_new_order_m_tmpl_id = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_seller_on_new_order_m_tmpl_id')
        seller_payment_journal_id = self.env['ir.default'].sudo()._get('res.config.settings', 'seller_payment_journal_id', company_id=True) or self.get_journal_id().id
        mp_currency_id = self.env['ir.default'].sudo()._get('res.config.settings', 'mp_currency_id') or self.env.user.company_id.currency_id.id
        show_visit_shop = self.env['ir.default'].sudo()._get('res.config.settings', 'show_visit_shop')
        group_mp_product_variant = self.env['ir.default'].sudo()._get('res.config.settings', 'group_mp_product_variant')
        group_mp_shop_allow = self.env['ir.default'].sudo()._get('res.config.settings', 'group_mp_shop_allow')
        group_mp_product_pricelist = self.env['ir.default'].sudo()._get('res.config.settings', 'group_mp_product_pricelist')
        group_mp_product_uom = self.env['ir.default'].sudo()._get('res.config.settings', 'group_mp_product_uom')
        seller_payment_product_id = self.env['ir.default'].sudo()._get('res.config.settings', 'seller_payment_product_id')
        
        enable_notify_admin_on_no_qty = self.env['ir.default'].sudo()._get('res.config.settings', 'enable_notify_admin_on_no_qty')
        notify_admin_on_no_qty = self.env['ir.default'].sudo()._get('res.config.settings', 'notify_admin_on_no_qty')
        commission_calculation_type = self.env['ir.default'].sudo()._get(
            'res.config.settings', 'commission_calculation_type')
        ...
        res.update(
            ...
            mp_payment_waiting_period = mp_payment_waiting_period,
    ...
    def get_mp_global_field_value(self, default_key):
        ...
        if default_key in company_fields:
            company = True
        field_value = self.env['ir.default'].sudo()._get('res.config.settings', default_key, company_id=company)
    ...
    def execute(self):
        ...
        if self.mp_payment_waiting_period < 0:
            raise UserError(_("Cashable Amount Due Date Gap can't be negative."))
